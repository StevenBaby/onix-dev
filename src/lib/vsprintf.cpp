...
// precision - 数字长度(精度)
// flags - 选项
static char *number(char *str, word_t *num, int base, int size, int precision, int flags)
{
    ...
    if (flags & DOUBLE)
    {
        word_t ival = (word_t)(*(double *)num);
        word_t fval = (word_t)(((*(double *)num) - ival) * 1000000);
        do
        {
            index = (fval) % base;
            (fval) /= base;
...
namespace arch
{
    int vsprintf(char *buf, const char *fmt, va_list args)
    {
        ...
        repeat:
            ...
            case 'p':
                // 此时若该参数没有设置宽度域，则默认宽度为 16，并且需要添零
                if (field_width == -1)
                {
                    field_width = sizeof(word_t) * 2;
                    flags |= ZEROPAD;
                }
                num = va_arg(args, word_t);
            ...
            case 'f':
                ...
                str = number(str, (word_t *)&dnum, 10, field_width, precision, flags);
                break;
            case 'b': // binary
                num = va_arg(args, unsigned long);
