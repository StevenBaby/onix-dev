...
static void get_screen(console_t *con)
{
    ...
    con->screen = con->mem_base + (high << 9 | low << 1);
}
...
static void get_cursor(console_t *con)
{
    ...
    outb(CRT_ADDR_REG, CRT_CURSOR_L); // 光标低地址
    u8 low = inb(CRT_DATA_REG);
    con->pos = con->mem_base + (high << 9 | low << 1);

    u64 offset = con->pos - con->screen;
    con->x = (offset % con->row_size) >> 1;
    con->y = (offset / con->row_size);
}
